...
#[tauri::command] fn copy_url(text:String)->Result<(),String>{let mut clipboard=arboard::Clipboard::new().map_err(|e|e.to_string())?;clipboard.set_text(text).map_err(|e|e.to_string())}
#[tauri::command] fn import_legacy(app:tauri::AppHandle)->Result<usize,String>{let path=storage::find_legacy_file().ok_or("MusicList.txt was not found beside the app. Place it beside the executable and try again.")?;let mut imported=storage::import_csv(&path)?;media::enrich_legacy_songs(&app,&mut imported)?;let count=imported.len();storage::save_songs(&app,&imported)?;storage::write_updated_csv(&path,&imported)?;Ok(count)}
