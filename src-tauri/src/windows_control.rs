...
mod platform {
    ...
    fn find_window(exe: &str) -> Result<HWND,String> { let mut data=FindData{wanted:exe.to_ascii_lowercase(),hwnd:HWND::default()}; unsafe{let _=EnumWindows(Some(enum_proc),LPARAM(&mut data as *mut _ as isize));} if data.hwnd.0==0{Err(format!("Could not find {exe}. Start GTA first or change the executable in Settings."))}else{Ok(data.hwnd)} }
